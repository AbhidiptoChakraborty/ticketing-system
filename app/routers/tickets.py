...
from app.services.audit import record_ticket_activity
...
async def _find_accessible_ticket(
    ticket_id: int,
    db: AsyncSession,
    current_user: User,
) -> Ticket:
...
async def create_ticket(
    ...
    background_tasks.add_task(
        send_notification,
        f"New {new_ticket.priority} ticket: {new_ticket.title} "
        f"(id={new_ticket.id})",
    )
    return new_ticket
...
async def list_tickets(
    ...
    order_by = {
        ...
        "priority": priority_rank.desc(),
    }[sort]
    ticket_query = (
        select(Ticket)
        .where(*filters)
        .order_by(order_by, Ticket.id.desc())
...
async def ticket_dashboard(
    ...
    priority_rows = await db.execute(priority_query)
...
async def update_ticket(
    ...
    for field, value in changes.items():
        setattr(
            ticket,
            field,
            value.upper() if field == "category" and value else value,
        )
    record_ticket_activity(
        db,
        ticket_id=ticket.id,
...
async def list_activity(
    ...
    """Return the chronological audit trail for a ticket."""
    await _find_accessible_ticket(ticket_id, db, current_user)
    activity_query = (
